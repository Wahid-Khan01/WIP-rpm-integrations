...
def create_user(uid=None, name=None, email=None, group='', reviewGroups=None, commentGroups=None, userInfoGroups=None,
                favorite=False, deniedPermissions=None, descriptions=None, templates=None, avatar=None):
    if not uid:
        uid='uid-1'
    if not name or not email:
        name="John Crook"
        email="jc@gmail.com"
    if not commentGroups:
        commentGroups = {}
    if not deniedPermissions:
        deniedPermissions = []
    if not descriptions:
        descriptions= descr_user_1
    return User(uid, name, email, group, reviewGroups, commentGroups, userInfoGroups, favorite, deniedPermissions,
                    descriptions, templates, avatar)